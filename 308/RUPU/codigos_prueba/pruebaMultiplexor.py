...
def main(args=None):
	...
	for i in range (50):
		for s in range(16):
			...
			valor = adc.read_adc(0, gain=GAIN, data_rate = sps)
	...
	while True:
		for s in range(16):
			...
			values[s] = round(((1-2*1)*100*(adc.read_adc(0, gain=GAIN, data_rate=sps)-minimo[s])/(maximo[s] - minimo[s])) + 100*1, 2)
