...
def calibrarSensores():		#calibrar sensores IR
	...
	while(abs(configuracion.radioRueda*pi*(configuracion.encoderD.read()- configuracion.encoderL.read())*configuracion.N/(configuracion.CPR*configuracion.l))<4*pi):
		...
		for s in range(16):
			...
			# valor = adc.read_adc(0, gain=configuracion.GAIN, data_rate=configuracion.sps)
			# valor = adc.read_adc(0, gain=configuracion.GAIN, data_rate=configuracion.sps)
			# valor = adc.read_adc(0, gain=configuracion.GAIN, data_rate=configuracion.sps)
			valor = adc.read_adc(0, gain=configuracion.GAIN, data_rate=configuracion.sps)
	...
	while(((1-2*configuracion.linea)*100*(adc.read_adc(0, gain=configuracion.GAIN, data_rate=configuracion.sps) - gl.minimo[2]) / (gl.maximo[2]-gl.minimo[2]) + 100*configuracion.linea ) < 80 ):
...
def obtenerPosicion (direccion):	#obtienela longitud de arco que existe entre la linea y el centro de los sensores, respecto al eje de giro
	...
	if (direccion == 1):	#direccion 0:hacia atras 1:hacia delante
		for s in range(8):	#mido los sensores IR de delante
			...
			# valor_adc = adc.read_adc(0, gain=configuracion.GAIN, data_rate=configuracion.sps)
			sensorNormalizado[s] = ((1-2*configuracion.linea)*100*(adc.read_adc(0, gain=configuracion.GAIN, data_rate=configuracion.sps)-gl.minimo[s])/(gl.maximo[s] -gl.minimo[s]))+100*configuracion.linea		#normaliza entre 0 a 100
		...
		for s in range(8):	#mido los sensores IR de atras
			...
			# valor_adc = adc.read_adc(0, gain=configuracion.GAIN, data_rate=configuracion.sps)
			# valor_adc = adc.read_adc(0, gain=configuracion.GAIN, data_rate=configuracion.sps)
			sensorNormalizado[s] = ((1-2*configuracion.linea)*100*(adc.read_adc(0, gain=configuracion.GAIN, data_rate=configuracion.sps)-gl.minimo[s+8])/(gl.maximo[s+8] -gl.minimo[s+8]))+100*configuracion.linea
			if (sensorNormalizado[s]<30):
				sensorNormalizado[s]=0
