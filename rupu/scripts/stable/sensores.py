...
# configuracion.CS1_PIN = 29  # Physical Pin 29 (for MCP3208 #1, channels 0-7)
# configuracion.CS2_PIN = 22  # Physical Pin 7 (for MCP3208 #2, channels 8-15)

# # Initialize GPIO
# # GPIO.setmode(GPIO.BOARD)  # Use BOARD mode
# GPIO.setwarnings(False)   # Suppress GPIO warnings
# GPIO.setup(configuracion.CS1_PIN, GPIO.OUT)
# GPIO.setup(configuracion.CS2_PIN, GPIO.OUT)

# # Set CS pins to high (inactive) initially
# GPIO.output(configuracion.CS1_PIN, GPIO.HIGH)
# GPIO.output(configuracion.CS2_PIN, GPIO.HIGH)
...
def calibrarSensores():		#calibrar sensores IR
	...
	while(abs(configuracion.radioRueda*3.14159265359*(configuracion.posR - configuracion.posL)*configuracion.N/(configuracion.CPR*configuracion.l))<4*3.14159265359):
		actuadores.motor(-vcal,vcal)
		if(gl.flag_debug):
			print(abs(configuracion.radioRueda*3.14159265359*(configuracion.posR - configuracion.posL)*configuracion.N/(configuracion.CPR*configuracion.l)))

		for s in range(16):
			
			if s<8:
				valor = read_adc(s, configuracion.CS1_PIN)
			else:
				valor = read_adc(s - 8, configuracion.CS2_PIN)
	...
	while(((1-2*configuracion.linea)*100*(read_adc(4, configuracion.CS1_PIN) - gl.minimo[2]) / (gl.maximo[2]-gl.minimo[2]) + 100*configuracion.linea ) < 80 ):
...
def distancia():				#mide la distancia 
	...
	# dist = tof.get_distance() #en cm, se le resta el offset
...
def velocidades():	#calcula velocidades, y la entrada del controlador PID de velocidad
	...
	if(gl.t_actual >= 0.01):	#cada 10 ms
		...
		v_r[0]=(2*3.14159265359*configuracion.radioRueda*configuracion.posR*configuracion.N)/(gl.t_actual*configuracion.CPR); # ENCODERS:28CPR---CAJA_REDUCTORA:100:1  [cm/s]
		v_l[0]=(2*3.14159265359*configuracion.radioRueda*configuracion.posL*configuracion.N)/(gl.t_actual*configuracion.CPR); # ENCODERS:28CPR---CAJA_REDUCTORA:100:1  [cm/s]
...
def obtenerPosicion (direccion):	#obtienela longitud de arco que existe entre la linea y el centro de los sensores, respecto al eje de giro
	...
	if (direccion == 1):	#direccion 0:hacia atras 1:hacia delante
		for s in range(8):	#mido los sensores IR de delante
			# t_inicio = time.time()
			sensorNormalizado[s] = ((1-2*configuracion.linea)*100*(read_adc(s, configuracion.CS1_PIN)-gl.minimo[s])/(gl.maximo[s] -gl.minimo[s]))+100*configuracion.linea		#normaliza entre 0 a 100
		...
		for s in range(8):	#mido los sensores IR de atras
			...
			# valor_adc = adc.read_adc(0, gain=configuracion.GAIN, data_rate=configuracion.sps)
			sensorNormalizado[s] = ((1-2*configuracion.linea)*100*(read_adc(s, configuracion.CS2_PIN)-gl.minimo[s+8])/(gl.maximo[s+8] -gl.minimo[s+8]))+100*configuracion.linea
			if (sensorNormalizado[s]<30):
				sensorNormalizado[s]=0.
